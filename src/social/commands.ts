...
export async function cmdSocialSetup(flags: SocialFlags): Promise<void> {
  ...
  if (flags.human) {
    ...
    process.stderr.write(`\nNext: capcut social-generate --platform twitter --topic "your topic"\n`);
  } else {
...
export async function cmdSocialGenerate(positional: string[], flags: SocialFlags): Promise<void> {
  const config = loadConfig();
  if (!config.brandVoice) die("No brand voice configured. Run: capcut social-setup");
  ...
  if (flags.human) {
    ...
    process.stderr.write(
      `\n${posts.length} post(s) generated as drafts.\n` +
        `Schedule with: capcut social-schedule --id <id> --at "2024-12-25 14:00"\n`,
    );
  } else {
    out(posts, flags);
  }
...
export function cmdSocialSchedule(positional: string[], flags: SocialFlags): void {
  ...
  if (!text) die("Provide post text or --id. Usage: capcut social-schedule <text> --platform <p> --at <datetime>");
...
export function cmdSocialStatus(flags: SocialFlags): void {
  ...
  if (flags.human) {
    ...
    if (!config.brandVoice) {
      process.stderr.write("Not configured. Run: capcut social-setup\n\n");
      return;
    }
